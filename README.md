...
- Web UI (FastAPI): страницы `/`, `/folders`, `/browse`.
...
- Дедупликация локальной папки для сортировки: **`C:\tmp\Photo`** (scope `source`) с использованием общего кеша хэшей.
...
- `http://127.0.0.1:8000/duplicates` — просмотр дублей (read-only)
