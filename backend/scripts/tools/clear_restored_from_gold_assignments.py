#!/usr/bin/env python3
"""
Удаление всех записей с source='restored_from_gold' из person_rectangle_manual_assignments.
...
cur.execute("SELECT COUNT(*) as count FROM person_rectangle_manual_assignments WHERE source = 'restored_from_gold'")
...
cur.execute("""
    ...
    WHERE source = 'restored_from_gold'
      AND EXISTS (
          ...
          WHERE fcm.rectangle_id = person_rectangle_manual_assignments.rectangle_id
            AND fc.person_id = person_rectangle_manual_assignments.person_id
      )
""")
conflict_count = cur.rowcount

# Удаляем все оставшиеся записи с source='restored_from_gold'
cur.execute("DELETE FROM person_rectangle_manual_assignments WHERE source = 'restored_from_gold'")
