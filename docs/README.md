...
- **Единая карточка фотографий** (`photo_card.js`): используется на всех страницах (faces.html, person_detail.html, face_cluster_detail.html) для просмотра и редактирования фото с rectangles. Поддерживает навигацию по списку, UNDO систему, создание новых rectangles (лицо/без лица), изменение типа привязки (кластер ↔ ручная), назначение персон через иерархическое меню. **Поворот фото**: для локальных изображений (путь `local:...`) в карточке доступны кнопки «повернуть влево» / «повернуть вправо» (рядом с кнопкой зума). При повороте изображение физически перезаписывается на диске (90° против/по часовой), EXIF Orientation сбрасывается, все прямоугольники (bbox) пересчитываются и сохраняются в БД; при повторном открытии карточки загружается актуальный файл (cache-bust по открытию).
