...
- **Переместить локально** — файлы перемещаются в целевые подпапки под корнем прогона; путь в БД обновляется одновременно.
- **Переместить в фотоархив** — файлы переносятся в `disk:/Фото/...`; файл перестаёт быть «сортируемым», в БД вносятся соответствующие правки.

**Шаг 4: важные замечания (чтобы не повторять грабли)**

- **Сортируемые файлы** — определяются **только** по `inventory_scope='source'` и `last_run_id=dedup_run_id` (прогон дедупликации). Не по «path под корнем»: на Windows формат путей (обратный/прямой слэш) даёт несовпадение `path LIKE root_like`, из-за чего в выборку попадало лишь часть файлов. Fill, step4-report и счётчики вкладок должны использовать один и тот же критерий (inventory_scope + last_run_id).
