...
- **Шаг 4 не создаёт _quarantine** — раскладка только по папкам фотоархива и группам в _no_faces (поездки, Мемы, Дом и ремонт и т.д.). Файлы с автокарантином (tiny_face и т.п.) не получают целевую папку; при привязке к персоне они раскладываются в папку этой персоны.
