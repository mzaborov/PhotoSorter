...
Реализована система распознавания лиц из фотобанка (Я.Диск `disk:/Фото`) с определением персон:

- **Сканирование лиц**: скрипт `backend/scripts/tools/face_scan.py` сканирует папки на Я.Диске, находит лица (YuNet), извлекает embeddings (ArcFace) и сохраняет в БД.
- **Кластеризация**: похожие лица группируются в кластеры (DBSCAN) для массовой разметки.
...
- **Web UI**:
  ...
  - `/persons` — список персон с статистикой
  - `/persons/{person_id}` — детали персоны: все файлы с этим человеком, похожие лица, управление аватаром
...
- Хранение: embeddings в БД (таблица `face_rectangles.embedding`), кластеры в `face_clusters`, назначения в `face_labels`
...
- **DD-015 — Удаление cluster_id из face_labels (устранение избыточности)**: `face_cluster_members` является единственным источником истины о том, в каком кластере находится лицо. Колонка `cluster_id` удалена из `face_labels` для устранения избыточности и рассинхронизации данных. `face_labels` хранит только связь лицо → персона (без привязки к кластеру). Кластер определяется через JOIN с `face_cluster_members` при необходимости. Это решение устраняет проблему, когда при объединении кластеров в `face_labels` оставались старые записи со старым `cluster_id`, что приводило к рассинхронизации (одно лицо могло иметь несколько записей в `face_labels` с разными `cluster_id`). Подробный план рефакторинга: `docs/REFACTORING_PLAN_remove_cluster_id_from_face_labels.md`.
