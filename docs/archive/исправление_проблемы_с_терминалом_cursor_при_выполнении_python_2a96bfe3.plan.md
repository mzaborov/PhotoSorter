...
## Этап 0: Фиксация проблемных команд (ПРИОРИТЕТ)

### 0.1 Простое логирование проблемных команд

**Процесс:**

1. Когда терминал сломается, пользователь сообщает последнюю команду
2. Агент добавляет строку в текстовый журнал напрямую (редактированием файла):
   ```
   YYYY-MM-DD HH:MM - <команда>
   ```

**Цель:** Фиксировать только проблемные команды, после которых терминал ломается

**Журнал:** `backend/scripts/debug/terminal_broken_commands.log`

**Формат:** Простой текстовый файл, одна строка на команду:
```
2026-01-24 15:45 - cd /c/Projects/PhotoSorter && git log --oneline -10 -- backend/web_api/templates/persons_list.html
```

**Преимущество:** Можно редактировать напрямую, не требуется выполнение команд

**Зафиксированные проблемные команды:**
- `cd /c/Projects/PhotoSorter && git log --oneline -10 -- backend/web_api/templates/persons_list.html` - терминал сломался после этой команды

### 0.2 Удаление workspaceStorage для чистого тестирования
...
**Когда терминал сломается:**

1. Пользователь сообщает агенту последнюю команду
2. Агент добавляет строку в журнал `backend/scripts/debug/terminal_broken_commands.log`:
   ```
   YYYY-MM-DD HH:MM - <команда>
   ```

**Для каждой проблемной команды зафиксировать:**

- Команда (точная строка)
...
**Просмотр журнала проблемных команд:**

```powershell
# Все проблемные команды
Get-Content 'backend/scripts/debug/terminal_broken_commands.log'
```

**Цель:** Собрать список проблемных команд и найти общие паттерны (слэши/кириллица/Python/другое)
